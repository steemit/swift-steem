/**
 Ordered dictionary, stand-in for boost::container::flat_map.
 - author: Lukas Kubanek <https://github.com/lukaskubanek/OrderedDictionary>
 */
...
public struct OrderedDictionary<Key: Hashable, Value>: BidirectionalCollection {
    ...
    public init<S: Sequence>(_ elements: S) where S.Element == Element {
        for (key, value) in elements {
            precondition(!self.containsKey(key), "Elements sequence contains duplicate keys")
            self[key] = value
        }
    }
    ...
    public func containsKey(_ key: Key) -> Bool {
        return self._orderedKeys.contains(key)
    }
    ...
    public mutating func updateValue(_ value: Value, forKey key: Key) -> Value? {
        if self._orderedKeys.contains(key) {
    ...
    public mutating func removeAll(keepingCapacity keepCapacity: Bool = false) {
        self._orderedKeys.removeAll(keepingCapacity: keepCapacity)
        self._keysToValues.removeAll(keepingCapacity: keepCapacity)
    }
    ...
    public subscript(position: Index) -> Element {
        precondition(self.indices.contains(position), "OrderedDictionary index is out of range")
    ...
    public func index(forKey key: Key) -> Index? {
        return self._orderedKeys.index(of: key)
    }
    ...
    public func canInsert(_ newElement: Element) -> Bool {
        return self.canInsert(key: newElement.key)
    }
    ...
    public mutating func insert(_ newElement: Element, at index: Index) {
        precondition(self.canInsert(key: newElement.key), "Cannot insert duplicate key in OrderedDictionary")
        precondition(self.canInsert(at: index), "Cannot insert at invalid index in OrderedDictionary")
    ...
    public func canUpdate(_ newElement: Element, at index: Index) -> Bool {
        ...
        return self._canUpdate(newElement, at: index, keyPresentAtIndex: &keyPresentAtIndex)
    }
    ...
    public mutating func update(_ newElement: Element, at index: Index) -> Element? {
        ...
        if !keyPresentAtIndex {
            self._keysToValues.removeValue(forKey: replacedElement.key)
        }

        // Store the new position of the key and the new value associated with the key
        self._orderedKeys[index] = key
        self._keysToValues[key] = value
    ...
    public mutating func remove(at index: Index) -> Element? {
        ...
        self._keysToValues.removeValue(forKey: element.key)

        return element
    }

    private func _canUpdate(_ newElement: Element, at index: Index, keyPresentAtIndex: inout Bool) -> Bool {
        precondition(self.indices.contains(index), "OrderedDictionary index is out of range")
    ...
    public mutating func sort(by areInIncreasingOrder: (Element, Element) -> Bool) {
        self._orderedKeys = self._sortedElements(by: areInIncreasingOrder).map { $0.key }
    }
    ...
    public func sorted(by areInIncreasingOrder: (Element, Element) -> Bool) -> OrderedDictionary<Key, Value> {
        return OrderedDictionary(self._sortedElements(by: areInIncreasingOrder))
    }
